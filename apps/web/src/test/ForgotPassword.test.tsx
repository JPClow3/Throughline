...
describe("ForgotPassword", () => {
  it("redirects if already authed", () => {
    vi.spyOn(AuthProvider, "useAuth").mockReturnValue({ status: "authed" } as unknown as ReturnType<typeof AuthProvider.useAuth>);
    const { container } = render(<MemoryRouter><ForgotPassword /></MemoryRouter>);
    expect(container.textContent).toBe("");
  });

  it("renders the form when anon", () => {
    vi.spyOn(AuthProvider, "useAuth").mockReturnValue({ status: "anon" } as unknown as ReturnType<typeof AuthProvider.useAuth>);
  ...
  it("calls resetPassword on submit", async () => {
    ...
    vi.spyOn(AuthProvider, "useAuth").mockReturnValue({ status: "anon", resetPassword } as unknown as ReturnType<typeof AuthProvider.useAuth>);
